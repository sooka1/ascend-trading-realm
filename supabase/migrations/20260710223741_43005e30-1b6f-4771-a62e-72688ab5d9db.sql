...
-- 1) Prevent negative subscription capital at the column level.
ALTER TABLE public.subscriptions
  DROP CONSTRAINT IF EXISTS subscriptions_amount_nonneg;
ALTER TABLE public.subscriptions
  ADD CONSTRAINT subscriptions_amount_nonneg CHECK (amount >= 0);

-- 2) Harden the withdrawal trigger: validate available balance up-front and
--    guarantee no subscription drops below zero during deduction.
...
DECLARE
  ...
  available    numeric;
BEGIN
  -- Wallet balance guard: approved deposits minus outstanding withdrawals
  -- (including this new pending one). Rejects overdrafts before any change.
  ...
  available := approved_in - pending_out;
  IF available < 0 THEN
