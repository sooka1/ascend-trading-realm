...
-- Redefine distribution to run per-day (weekly rate / 5 business days)
...
BEGIN
  ...
  SELECT s.user_id,
         s.id,
         ROUND((s.amount * COALESCE(p.target_return_pct, 0) / 100.0 / 52.0 / 5.0)::numeric, 2),
...
-- Reschedule: unschedule weekly, schedule daily Mon–Fri at 00:05 UTC
DO $$
BEGIN
...
DO $$
BEGIN
  PERFORM cron.unschedule('distribute-daily-profits')
  WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'distribute-daily-profits');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;
